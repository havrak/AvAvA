  ...
  if (data.status_code != 102) {
    ...
    if (data.network)
      Object.keys(data.network).forEach((key) => {
        ...
        counters.upload.usedSpeed = lxdc.bytes_sent - counters.upload.bytesSent;
...
export function deleteProject(id) {
  return new Promise((resolve) => {
    deleteImagesInProject(id).then((result) => {
      deleteProfilesInProject(id).then((result) => {
...
export function deleteImagesInProject(id) {
  return new Promise((resolve) => {
    mkRequest(`/1.0/images?project=p${id}`).then((res) => {
      if (!res || res.error) {
        resolve(getOperation(res));
      } else {
        let counter = 0;
...
// will delete all profiles with exception of default
export function deleteProfilesInProject(id) {
  return new Promise((resolve) => {
    mkRequest(`/1.0/profiles?project=p${id}`).then((res) => {
      if (!res || res.error) {
        resolve(getOperation(res));
      } else {
        let counter = 0;
        ...
        if (res.length == 0) {
          resolve(new OperationState("successfully deleted all profiles"), 200);
        }
      }
    });
  });
