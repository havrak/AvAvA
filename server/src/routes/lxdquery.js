...
export function execInstance(id, project, command, getOutput, haproxy) {
  ...
  if (haproxy == true) {
    path = `/1.0/instances/${id}/exec?project=${project}`;
  }
...
function postToInstance(id, project, piper, dstPath, headers, proxy) {
...
export function postFileToInstance(id, project, srcPath, dstPath, proxy) {
