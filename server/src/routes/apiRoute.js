			...
			if (result) {
				containerSQL
					...
					.then((result) => {
						...
						lxd.createInstance(result, result.appsToInstall).then(
							(result) => {
	...
	(req, res) =>
		containerSQL
			...
			.then((result) => {
				if (result.statusCode)
					res.status(400).send({ message: result.status });
				lxd.exportInstance(req.params.instanceId, result, res).then(
					(result) => {
						if (result.statusCode && result.statusCode != 200)
							res.status(400).send({ message: result.status });
					}
...
app.post("/api/projects", isLoggedIn, (req, res) => {
	projectSQL
		...
		.then((project) => {
			if (project.statusCode && project.statusCode != 200) {
